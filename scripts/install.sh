...
TEMP_DIR="$(mktemp -d -t traderclaw-install-XXXXXX)"
cleanup() {
  rm -rf "$TEMP_DIR"
}
trap cleanup EXIT
...
if [[ -x "$TEMP_DIR/traderclaw_install.sh" ]]; then
  exec "$TEMP_DIR/traderclaw_install.sh" "$@"
fi
...
echo "error: traderclaw_install.sh/bootstrap.sh was not found in the fetched revision." >&2
echo "Run the local bootstrap directly when possible:" >&2
echo "  ./traderclaw_install.sh --help" >&2
exit 1
