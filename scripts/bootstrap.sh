...
usage() {
  cat <<'USAGE'
TraderClaw installer bootstrap engine

Usage:
  ./traderclaw_install.sh [options]
  ./bootstrap.sh [options]         # compatibility entrypoint

Modes:
  Default mode installs/builds TraderClaw only (requires existing Rust toolchain).
...
Examples:
  ./traderclaw_install.sh
  ./traderclaw_install.sh --guided
  ./traderclaw_install.sh --install-system-deps --install-rust
  ./traderclaw_install.sh --prefer-prebuilt
  ./traderclaw_install.sh --prebuilt-only
  ./traderclaw_install.sh --onboard --api-key "sk-..." --provider openrouter [--model "openrouter/auto"]
  ./traderclaw_install.sh --interactive-onboard

  # Compatibility entrypoint:
  ./bootstrap.sh --docker

  # Remote one-liner
  curl -fsSL https://raw.githubusercontent.com/traderclaw-labs/traderclaw/master/scripts/bootstrap.sh | bash

Environment:
  TRADERCLAW_CONTAINER_CLI     Container CLI command (default: docker; auto-fallback: podman)
  TRADERCLAW_DOCKER_DATA_DIR   Host path for Docker config/workspace persistence
  TRADERCLAW_DOCKER_IMAGE      Docker image tag to build/run (default: traderclaw-bootstrap:local)
  TRADERCLAW_API_KEY           Used when --api-key is not provided
  TRADERCLAW_PROVIDER          Used when --provider is not provided (default: openrouter)
  TRADERCLAW_MODEL             Used when --model is not provided
  TRADERCLAW_BOOTSTRAP_MIN_RAM_MB   Minimum RAM threshold for source build preflight (default: 2048)
  TRADERCLAW_BOOTSTRAP_MIN_DISK_MB  Minimum free disk threshold for source build preflight (default: 6144)
  TRADERCLAW_DISABLE_ALPINE_AUTO_DEPS
...
should_attempt_prebuilt_for_resources() {
  ...
  min_ram_mb="${TRADERCLAW_BOOTSTRAP_MIN_RAM_MB:-2048}"
  min_disk_mb="${TRADERCLAW_BOOTSTRAP_MIN_DISK_MB:-6144}"
...
install_prebuilt_binary() {
  ...
  extracted_bin="$temp_dir/traderclaw"
  if [[ ! -x "$extracted_bin" ]]; then
    extracted_bin="$(find "$temp_dir" -maxdepth 2 -type f -name traderclaw -perm -u+x | head -n 1 || true)"
  fi
  if [[ -z "$extracted_bin" || ! -x "$extracted_bin" ]]; then
    warn "Archive did not contain an executable traderclaw binary."
    rm -rf "$temp_dir"
    return 1
  fi
  ...
  mkdir -p "$install_dir"
  install -m 0755 "$extracted_bin" "$install_dir/traderclaw"
  rm -rf "$temp_dir"

  info "Installed pre-built binary to $install_dir/traderclaw"
...
run_pacman() {
  ...
  local pacman_rc=0
  pacman_cfg_tmp="$(mktemp /tmp/traderclaw-pacman.XXXXXX.conf)"
...
run_guided_installer() {
  ...
  echo
  echo "TraderClaw guided installer"
  ...
  if prompt_yes_no "Install traderclaw into cargo bin now?" "yes"; then
    SKIP_INSTALL=false
  else
    SKIP_INSTALL=true
  fi
...
resolve_container_cli() {
  local requested_cli
  requested_cli="${TRADERCLAW_CONTAINER_CLI:-docker}"
  ...
  if [[ "$requested_cli" != "docker" ]]; then
    error "Set TRADERCLAW_CONTAINER_CLI to an installed Docker-compatible CLI (e.g., docker or podman)."
  else
    error "Install Docker, install podman, or set TRADERCLAW_CONTAINER_CLI to an available Docker-compatible CLI."
  fi
  exit 1
}
...
run_docker_bootstrap() {
  ...
  docker_image="${TRADERCLAW_DOCKER_IMAGE:-traderclaw-bootstrap:local}"
  fallback_image="ghcr.io/traderclaw-labs/traderclaw:latest"
  if [[ "$TEMP_CLONE" == true ]]; then
    default_data_dir="$HOME/.traderclaw-docker"
  else
    default_data_dir="$WORK_DIR/.traderclaw-docker"
  fi
  docker_data_dir="${TRADERCLAW_DOCKER_DATA_DIR:-$default_data_dir}"
  DOCKER_DATA_DIR="$docker_data_dir"

  mkdir -p "$docker_data_dir/.traderclaw" "$docker_data_dir/workspace"
  ...
  if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if ! "$CONTAINER_CLI" image inspect "$docker_image" >/dev/null 2>&1; then
      ...
      info "Pulling official TraderClaw image ($fallback_image)"
  ...
  config_mount="$docker_data_dir/.traderclaw:/traderclaw-data/.traderclaw"
  workspace_mount="$docker_data_dir/workspace:/traderclaw-data/workspace"
...
Use either:
  --api-key "sk-..."
or:
  TRADERCLAW_API_KEY="sk-..." ./traderclaw_install.sh --docker
or run interactive:
  ./traderclaw_install.sh --docker --interactive-onboard
MSG
      exit 1
    fi
    if [[ -n "$MODEL" ]]; then
  ...
  "$CONTAINER_CLI" run --rm -it \
    ...
    -e HOME=/traderclaw-data \
    -e TRADERCLAW_WORKSPACE=/traderclaw-data/workspace \
    -v "$config_mount" \
    -v "$workspace_mount" \
    "$docker_image" \
    "${onboard_cmd[@]}"
...
CONTAINER_CLI="${TRADERCLAW_CONTAINER_CLI:-docker}"
API_KEY="${TRADERCLAW_API_KEY:-}"
PROVIDER="${TRADERCLAW_PROVIDER:-openrouter}"
MODEL="${TRADERCLAW_MODEL:-}"
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  if [[ "$OS_NAME" == "Linux" && -z "${TRADERCLAW_DISABLE_ALPINE_AUTO_DEPS:-}" ]] && have_cmd apk; then
    find_missing_alpine_prereqs
    if [[ ${#ALPINE_MISSING_PKGS[@]} -gt 0 && "$INSTALL_SYSTEM_DEPS" == false ]]; then
      ...
      info "Auto-enabling system dependency installation (set TRADERCLAW_DISABLE_ALPINE_AUTO_DEPS=1 to disable)."
      INSTALL_SYSTEM_DEPS=true
    fi
  fi
...
if [[ ! -f "$WORK_DIR/Cargo.toml" ]]; then
  if [[ -f "$(pwd)/Cargo.toml" ]]; then
    ...
    TEMP_DIR="$(mktemp -d -t traderclaw-bootstrap-XXXXXX)"
...
Your containerized TraderClaw data is persisted under:
DONE
...
Next steps:
  ./traderclaw_install.sh --docker --interactive-onboard
  ./traderclaw_install.sh --docker --api-key "sk-..." --provider openrouter
DONE
  exit 0
fi
...
or re-run with:
  ./traderclaw_install.sh --install-rust
MSG
  exit 1
fi
...
if [[ "$SKIP_INSTALL" == false ]]; then
  info "Installing traderclaw to cargo bin"
...
TRADERCLAW_BIN=""
if have_cmd traderclaw; then
  TRADERCLAW_BIN="traderclaw"
elif [[ -x "$HOME/.cargo/bin/traderclaw" ]]; then
  TRADERCLAW_BIN="$HOME/.cargo/bin/traderclaw"
elif [[ -x "$WORK_DIR/target/release/traderclaw" ]]; then
  TRADERCLAW_BIN="$WORK_DIR/target/release/traderclaw"
fi

if [[ "$RUN_ONBOARD" == true ]]; then
  if [[ -z "$TRADERCLAW_BIN" ]]; then
    error "onboarding requested but traderclaw binary is not available."
    error "Run without --skip-install, or ensure traderclaw is in PATH."
    exit 1
  fi

  if [[ "$INTERACTIVE_ONBOARD" == true ]]; then
    info "Running interactive onboarding"
    "$TRADERCLAW_BIN" onboard --interactive
  else
    if [[ -z "$API_KEY" ]]; then
      cat <<'MSG'
==> Onboarding requested, but API key not provided.
Use either:
  --api-key "sk-..."
or:
  TRADERCLAW_API_KEY="sk-..." ./traderclaw_install.sh --onboard
or run interactive:
  ./traderclaw_install.sh --interactive-onboard
MSG
      exit 1
    ...
    ONBOARD_CMD=("$TRADERCLAW_BIN" onboard --api-key "$API_KEY" --provider "$PROVIDER")
    if [[ -n "$MODEL" ]]; then
      ONBOARD_CMD+=(--model "$MODEL")
    fi
    "${ONBOARD_CMD[@]}"
...
Next steps:
  traderclaw status
  traderclaw agent -m "Hello, TraderClaw!"
  traderclaw gateway
DONE
