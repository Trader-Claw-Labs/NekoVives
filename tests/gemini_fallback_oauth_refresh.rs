...
//!
//! Requires:
//! - Live Gemini OAuth profile in `~/.traderclaw/auth-profiles.json` with refresh_token
...
async fn gemini_warmup_refreshes_expired_oauth_token() -> Result<()> {
    // Find ~/.traderclaw/auth-profiles.json
    let home = env::var("HOME").expect("HOME env var not set");
    let traderclaw_dir = PathBuf::from(home).join(".traderclaw");
    let auth_profiles_path = traderclaw_dir.join("auth-profiles.json");
    ...
    if !auth_profiles_path.exists() {
        ...
        eprintln!("Run: traderclaw auth login --provider gemini");
        return Ok(());
    }
    ...
    let gemini_profile_key = profiles
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No Gemini OAuth profile found. Run: traderclaw auth login --provider gemini"
            )
        })?
        .clone();
    ...
    // This will load auth from ~/.traderclaw/auth-profiles.json (with expired token)
    let provider = traderclaw::providers::create_provider("gemini", None)?;
