...
TraderClaw implements defense-in-depth security:
...
TraderClaw Docker images follow CIS Docker Benchmark best practices:
...
docker build -t traderclaw .
docker inspect --format='{{.Config.User}}' traderclaw
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace traderclaw gateway
```
