# Adding Boards and Tools — TraderClaw Hardware Guide

This guide explains how to add new hardware boards and custom tools to TraderClaw.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.traderclaw/config.toml)
traderclaw peripheral add nucleo-f401re /dev/ttyACM0
traderclaw peripheral add arduino-uno /dev/cu.usbmodem12345
traderclaw peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, TraderClaw can index PDF files:
...
2. **Add to config** — `traderclaw peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `traderclaw peripheral list` | List configured boards |
| `traderclaw peripheral add <board> <path>` | Add board (writes config) |
| `traderclaw peripheral flash` | Flash Arduino firmware |
| `traderclaw peripheral flash-nucleo` | Flash Nucleo firmware |
| `traderclaw hardware discover` | List USB devices |
| `traderclaw hardware info` | Chip info via probe-rs |
