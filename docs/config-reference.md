# TraderClaw Config Reference (Operator-Oriented)
...
1. `TRADERCLAW_WORKSPACE` override (if set)
2. persisted `~/.traderclaw/active_workspace.toml` marker (if present)
3. default `~/.traderclaw/config.toml`

TraderClaw logs the resolved config on startup at `INFO` level:

- `Config loaded` with fields: `path`, `workspace`, `source`, `initialized`

Schema export command:

- `traderclaw config schema` (prints JSON Schema draft 2020-12 to stdout)
...
| `otel_service_name` | `traderclaw` | Service name emitted to OTLP collector |
...
- Query runtime traces with:
  - `traderclaw doctor traces --limit 20`
  - `traderclaw doctor traces --event tool_call_result --contains \"error\"`
  - `traderclaw doctor traces --id <trace-id>`
...
1. `TRADERCLAW_PROVIDER` (explicit override, always wins when non-empty)
...
- Use `TRADERCLAW_PROVIDER` when you intentionally want runtime env to override a non-default configured provider.
...
- When `enabled = true` and no OTP secret exists, TraderClaw generates one and prints an enrollment URI once.
...
| `state_file` | `~/.traderclaw/estop-state.json` | Persistent estop state path |
...
- Use CLI command `traderclaw estop` to engage and `traderclaw estop resume` to clear levels.
...
- Security-first default: TraderClaw does **not** clone or sync `open-skills` unless `open_skills_enabled = true`.
- Environment overrides:
  - `TRADERCLAW_OPEN_SKILLS_ENABLED` accepts `1/0`, `true/false`, `yes/no`, `on/off`.
  - `TRADERCLAW_OPEN_SKILLS_DIR` overrides the repository path when non-empty.
  - `TRADERCLAW_SKILLS_PROMPT_MODE` accepts `full` or `compact`.
- Precedence for enable flag: `TRADERCLAW_OPEN_SKILLS_ENABLED` → `skills.open_skills_enabled` in `config.toml` → default `false`.
...
- Skill loading and `traderclaw skills install` both apply a static security audit. Skills that contain symlinks, script-like files, high-risk shell payload snippets, or unsafe markdown link traversal are rejected.
...
- TraderClaw requests Composio v3 tools with `toolkit_versions=latest` and executes tools with `version="latest"` to avoid stale default tool revisions.
...
3. Validate with `traderclaw doctor` before restart/rollout.
...
- While `traderclaw channel start` is running, updates to `default_provider`, `default_model`, `default_temperature`, `api_key`, `api_url`, and `reliability.*` are hot-applied from `config.toml` on the next inbound message.
...
- `TRADERCLAW_LINQ_SIGNING_SECRET` overrides `signing_secret` when set.
...
- `TRADERCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides `webhook_secret` when set.
