...
This guide covers native Nextcloud Talk integration for TraderClaw.
...
Add this section in `~/.traderclaw/config.toml`:
...
traderclaw daemon
# or
traderclaw gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, TraderClaw verifies:
...
- TraderClaw ignores bot-originated webhook events (`actorType = bots`).
- TraderClaw ignores non-message/system events.
...
3. Confirm TraderClaw receives and replies in the same room.
