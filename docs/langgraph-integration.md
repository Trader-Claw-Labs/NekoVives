...
This guide explains how to use the `traderclaw-tools` Python package for consistent tool calling with any OpenAI-compatible LLM provider.

## Background

Some LLM providers, particularly Chinese models like GLM-5 (Zhipu AI), have inconsistent tool calling behavior when using text-based tool invocation. TraderClaw's Rust core uses structured tool calling via the OpenAI API format, but some models respond better to a different approach.
...
```python
import asyncio
from traderclaw_tools import create_agent, shell, file_read, file_write
...
```python
from traderclaw_tools import tool, create_agent
...
```python
import os
from traderclaw_tools.integrations import DiscordBot
...
# Single message
traderclaw-tools "What is the current date?"

# Interactive mode
traderclaw-tools -i
```

## Comparison with Rust TraderClaw

| Aspect | Rust TraderClaw | traderclaw-tools |
|--------|---------------|-----------------|
