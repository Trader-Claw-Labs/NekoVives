# TraderClaw Security Improvement Roadmap
...
TraderClaw already has **excellent application-layer security**:
...
## Comparison: TraderClaw vs PicoClaw vs Production Grade

| Feature | PicoClaw | TraderClaw Now | TraderClaw + Roadmap | Production Target |
|---------|----------|--------------|-------------------|-------------------|
...
| Security self-test (`traderclaw audit --check`) | 1 day | Low |
...
[security.audit]
enabled = true
log_path = "~/.config/traderclaw/audit.log"
sign_events = true
max_size_mb = 100
...
traderclaw security --check
# → ✓ Sandbox: Firejail active
# → ✓ Audit logging enabled (42 events today)
# → → Resource limits: 512MB mem, 50% CPU

# Audit log queries
traderclaw audit --user @alice --since 24h
traderclaw audit --risk high --violations-only
traderclaw audit --verify-signatures

# Sandbox test
traderclaw sandbox --test
# → Testing isolation...
...
**TraderClaw is already more secure than PicoClaw** with:
...
**By implementing this roadmap**, TraderClaw becomes:
...
**Value**: Transforms TraderClaw from "safe for testing" to "safe for production"
