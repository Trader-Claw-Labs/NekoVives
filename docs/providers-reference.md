# TraderClaw Providers Reference
...
3. Generic fallback env vars: `TRADERCLAW_API_KEY` then `API_KEY`
...
- After multimodal normalization, TraderClaw sends image payloads through Ollama's native `messages[].images` field.
- If a non-vision provider is selected, TraderClaw returns a structured capability error instead of silently ignoring images.
...
- TraderClaw normalizes a trailing `/api` in `api_url` automatically.
...
- Model discovery: `traderclaw models refresh --provider llamacpp`
...
- Model discovery: `traderclaw models refresh --provider sglang`
...
- Model discovery: `traderclaw models refresh --provider vllm`
...
- Model discovery: `traderclaw models refresh --provider osaurus`
...
- Model discovery: `traderclaw models refresh --provider nvidia`
