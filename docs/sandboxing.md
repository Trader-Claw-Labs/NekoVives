# TraderClaw Sandboxing Strategies
...
## Problem
TraderClaw currently has application-layer security (allowlists, path blocking, command injection protection) but lacks OS-level containment. If an attacker is on the allowlist, they can run any allowed command with traderclaw's user permissions.
...
readwrite_paths = ["$HOME/workspace", "/tmp/traderclaw"]
```
