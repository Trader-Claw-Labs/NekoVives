...
- `cargo check` / `cargo build` appears stuck at `Checking traderclaw` for a long time
- repeated `Blocking waiting for file lock on package cache` or `build directory`

Why this happens in TraderClaw:
...
### `traderclaw` command not found after install

Symptom:

- install succeeds but shell cannot find `traderclaw`
...
- stop extra `traderclaw daemon` / `traderclaw channel start` processes
