...
TraderClaw supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
# or: export TRADERCLAW_API_KEY="your-api-key"
traderclaw agent
```

## llama.cpp Server (Recommended Local Setup)

TraderClaw includes a first-class local provider for `llama-server`:
...
```bash
traderclaw models refresh --provider llamacpp
traderclaw agent -m "hello"
```

You do not need to export `TRADERCLAW_API_KEY=dummy` for this flow.

## SGLang Server

TraderClaw includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
```bash
traderclaw models refresh --provider sglang
traderclaw agent -m "hello"
```
...
TraderClaw includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
```bash
traderclaw models refresh --provider vllm
traderclaw agent -m "hello"
```
...
# Single message test
traderclaw agent -m "test message"
```
