...
[INFO] ✓ Audit logging enabled (~/.config/traderclaw/audit.log)
...
```bash
# Check what's active
$ traderclaw security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/traderclaw/audit.log
  → 47 events logged today

# Disable sandbox explicitly (writes to config)
$ traderclaw config set security.sandbox.enabled false

# Enable specific backend
$ traderclaw config set security.sandbox.backend firejail

# Adjust limits
$ traderclaw config set security.resources.max_memory_mb 2048
```
...
```bash
$ traderclaw onboard --security-level paranoid
[1/9] Workspace Setup...
...
...
✅ **Status visibility** — `traderclaw security --status`
