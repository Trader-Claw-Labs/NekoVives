...
This page defines the fastest supported path to install and initialize TraderClaw.
...
Default behavior is **app-only** (build/install TraderClaw) and expects existing Rust toolchain.
...
```bash
curl -fsSL https://raw.githubusercontent.com/traderclaw-labs/traderclaw/main/scripts/bootstrap.sh | bash
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/traderclaw-labs/traderclaw/main/scripts/install.sh | bash
```
...
This builds a local TraderClaw image and launches onboarding inside a container while
persisting config/workspace to `./.traderclaw-docker`.

Container CLI defaults to `docker`. If Docker CLI is unavailable and `podman` exists,
bootstrap auto-falls back to `podman`. You can also set `TRADERCLAW_CONTAINER_CLI`
explicitly (for example: `TRADERCLAW_CONTAINER_CLI=podman ./bootstrap.sh --docker`).
...
Docker tag (`TRADERCLAW_DOCKER_IMAGE`, default: `traderclaw-bootstrap:local`); if missing,
it pulls `ghcr.io/traderclaw-labs/traderclaw:latest` and tags it locally before running.
...
```bash
TRADERCLAW_API_KEY="sk-..." TRADERCLAW_PROVIDER="openrouter" ./bootstrap.sh --onboard
```
...
- `--install-system-deps`
- `--install-rust`
- `--skip-build` (in `--docker` mode: use local image if present, otherwise pull `ghcr.io/traderclaw-labs/traderclaw:latest`)
- `--skip-install`
- `--provider <id>`
