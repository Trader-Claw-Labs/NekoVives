...
This document is the canonical reference for channel configuration in TraderClaw.
...
6. **Stale runtime state**: config changed but `traderclaw daemon` was not restarted.

---

## 1. Configuration Namespace

All channel settings live under `channels_config` in `~/.traderclaw/config.toml`.
...
When running `traderclaw channel start` (or daemon mode), Telegram and Discord now support sender-scoped runtime switching:
...
- Model cache previews come from `traderclaw models refresh --provider <ID>`.
- These are runtime chat commands, not CLI subcommands.

## Inbound Image Marker Protocol

TraderClaw supports multimodal input through inline message markers:
...
If `[channels_config.matrix]`, `[channels_config.lark]`, or `[channels_config.feishu]` is present but the corresponding feature is not compiled in, `traderclaw channel list`, `traderclaw channel doctor`, and `traderclaw channel start` will report that the channel is intentionally skipped for this build.
...
access_token = "syt_..."
user_id = "@traderclaw:matrix.example.com"   # optional, recommended for E2EE
...
TraderClaw supports two WhatsApp backends:
...
```toml
[channels_config.whatsapp]
session_path = "~/.traderclaw/state/whatsapp-web/session.db"
...
port = 6697
nickname = "traderclaw-bot"
username = "traderclaw"              # optional
channels = ["#traderclaw"]
allowed_users = ["*"]
...
- `TRADERCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides config secret.
...
- `TRADERCLAW_LINQ_SIGNING_SECRET` overrides config secret.
...
5. Restart `traderclaw daemon` after config changes.
...
```bash
RUST_LOG=info traderclaw daemon 2>&1 | tee /tmp/traderclaw.log
```

Then filter channel/gateway events:

```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Nostr|Webhook|Channel" /tmp/traderclaw.log
```
