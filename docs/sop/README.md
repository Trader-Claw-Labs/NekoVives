...
- CLI `traderclaw sop` currently manages definitions only: `list`, `validate`, `show`.
...
2. Create a SOP directory, for example:
   ...
   ```text
   ~/.traderclaw/workspace/sops/deploy-prod/SOP.toml
   ~/.traderclaw/workspace/sops/deploy-prod/SOP.md
   ```
...
3. Validate and inspect definitions:
   ...
   ```bash
   traderclaw sop list
   traderclaw sop validate
   traderclaw sop show deploy-prod
   ```
