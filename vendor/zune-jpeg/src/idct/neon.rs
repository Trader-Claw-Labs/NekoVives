...
pub unsafe fn idct_neon(
    ...
    if or_tree.all_zero() {
        ...
        let idct_value = unsafe { vdupq_n_s16(coeff) };
...
unsafe fn clamp_neon(reg: int16x8_t) -> int16x8_t {
    let min_s = unsafe { vdupq_n_s16(0) };
    let max_s = unsafe { vdupq_n_s16(255) };

    let max_v = unsafe { vmaxq_s16(reg, min_s) }; //max(a,0)
    let min_v = unsafe { vminq_s16(max_v, max_s) }; //min(max(a,0),255)
    min_v
}
