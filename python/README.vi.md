...
```bash
pip install traderclaw-tools[discord]
```
...
# Chạy CLI
traderclaw-tools "List files in the current directory"
...
```python
from traderclaw_tools import tool
...
## So sánh với Rust TraderClaw

| Tính năng | Rust TraderClaw | traderclaw-tools |
|---------|---------------|----------------|
...
Dùng **Rust TraderClaw** cho triển khai biên (edge) trong sản phẩm. Dùng **traderclaw-tools** khi cần đảm bảo tính nhất quán gọi công cụ hoặc tích hợp hệ sinh thái Python.
