//! Flash TraderClaw Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash TraderClaw Nucleo firmware. Builds from firmware/traderclaw-nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (traderclaw's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("traderclaw-nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from traderclaw repo root.",
            firmware_dir.display()
        );
    }

    println!("Building TraderClaw Nucleo firmware...");
    ...
    println!("TraderClaw Nucleo firmware flashed successfully.");
