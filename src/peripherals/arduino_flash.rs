//! Flash TraderClaw Arduino firmware via arduino-cli.
//!
...
/// TraderClaw Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
const FIRMWARE_INO: &str = include_str!("../../firmware/traderclaw-arduino/traderclaw-arduino.ino");

const FQBN: &str = "arduino:avr:uno";
const SKETCH_NAME: &str = "traderclaw-arduino";
...
/// Flash TraderClaw firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("traderclaw_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling TraderClaw Arduino firmware...");
    ...
    println!("TraderClaw firmware flashed successfully.");
