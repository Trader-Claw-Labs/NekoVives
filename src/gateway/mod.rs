...
pub async fn run_gateway(host: &str, port: u16, config: Config) -> Result<()> {
    ...
    let app = Router::new()
        ...
        .route("/api/capital/allocator", get(api::handle_api_capital_allocator))
        // ── Backtesting ──
        ...
        .route("/api/live/strategies/{id}/sync-onchain", post(api::handle_api_live_sync_onchain))
        .route("/api/live/stop-all-live", post(api::handle_api_live_stop_all))
        // ── Tick Recorder ──
...
mod tests {
    ...
    async fn metrics_endpoint_renders_prometheus_output() {
        ...
        let body = response.into_body().collect().await.unwrap().to_bytes();
        let text = String::from_utf8(body.to_vec()).unwrap();
        assert!(text.contains("traderclaw_heartbeat_ticks_total 1"));
    }
