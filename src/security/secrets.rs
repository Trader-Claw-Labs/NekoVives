...
// in `~/.traderclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.traderclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(traderclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: traderclaw_dir.join(".secret_key"),
            enabled,
        }
    }
