...
const SERVICE_LABEL: &str = "com.traderclaw.daemon";
const WINDOWS_TASK_NAME: &str = "TraderClaw Daemon";
...
fn start_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            ...
            run_checked(Command::new("systemctl").args(["--user", "start", "traderclaw.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["traderclaw", "start"]))?;
        }
...
fn stop_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            let _ =
                run_checked(Command::new("systemctl").args(["--user", "stop", "traderclaw.service"]));
        }
        InitSystem::Openrc => {
            let _ = run_checked(Command::new("rc-service").args(["traderclaw", "stop"]));
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service stopped");
...
fn restart_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]))?;
            run_checked(Command::new("systemctl").args(["--user", "restart", "traderclaw.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["traderclaw", "restart"]))?;
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service restarted");
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["traderclaw", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/traderclaw");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/traderclaw");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "traderclaw", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove traderclaw from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✅ Service uninstalled (/etc/init.d/traderclaw)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
"#,
    ...
    println!("   Start with: traderclaw service start");
    Ok(())
}
...
fn install_linux_systemd(config: &Config) -> Result<()> {
    ...
    let exe = std::env::current_exe().context("Failed to resolve current executable")?;
    let unit = format!(
        "[Unit]\nDescription=TraderClaw daemon\nAfter=network.target\n\n[Service]\nType=simple\nExecStart={} daemon\nRestart=always\nRestartSec=3\n\n[Install]\nWantedBy=default.target\n",
        exe.display()
    );

    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "traderclaw.service"]));
...
/// Check if the traderclaw user exists and has expected properties.
...
fn check_traderclaw_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "traderclaw"]).output();
    let is_alpine = Path::new("/etc/alpine-release").exists();

    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser traderclaw && delgroup traderclaw",
            "addgroup -S traderclaw && adduser -S -s /sbin/nologin -H -D -G traderclaw traderclaw",
        )
    } else {
        ("userdel traderclaw", "useradd -r -s /sbin/nologin traderclaw")
    };
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if uid.parse::<u32>().unwrap_or(999) >= 1000 {
                    bail!(
                        "User 'traderclaw' exists but has unexpected UID {} (expected system UID < 1000).\n\
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'traderclaw' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/traderclaw" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: traderclaw user has home directory '{}' (expected /var/lib/traderclaw or /nonexistent)",
                        home
                    );
                }
...
fn ensure_traderclaw_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "traderclaw"]).output();
    if let Ok(output) = output {
        if output.status.success() {
            return check_traderclaw_user();
        }
    }
    ...
    if is_alpine {
        let group_output = Command::new("getent").args(["group", "traderclaw"]).output();
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "traderclaw"])
                .output()
                .context("Failed to create traderclaw group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create traderclaw group: {}", stderr.trim());
            }
            println!("✅ Created system group: traderclaw");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create traderclaw user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create traderclaw user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "traderclaw"])
            .output()
            .context("Failed to create traderclaw user")?;
    ...
    println!("✅ Created system user: traderclaw");
    Ok(())
}

/// Change ownership of a path to traderclaw:traderclaw
#[cfg(unix)]
fn chown_to_traderclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["traderclaw:traderclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to traderclaw:traderclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_traderclaw(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_traderclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "traderclaw:traderclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to traderclaw:traderclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_traderclaw(_path: &Path) -> Result<()> {
    Ok(())
}
    ...
    if let Some(user) = sudo_user {
        if let Ok(output) = Command::new("getent").args(["passwd", &user]).output() {
            if output.status.success() {
                ...
                if fields.len() >= 6 {
                    return Some(PathBuf::from(fields[5]).join(".traderclaw"));
                }
            }
        }
    }
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'traderclaw' cannot write {} ({details}). \
             Re-run `sudo traderclaw service install` and ensure ownership is traderclaw:traderclaw.",
            path.display(),
        );
    }
    ...
    if path_str.contains("/home/") || path_str.contains(".cargo/bin") {
        eprintln!(
            "⚠️  Warning: Binary path '{}' appears to be in a user home directory.\n\
             ...
             sudo cp '{}' /usr/local/bin/traderclaw",
            exe_path.display(),
            exe_path.display()
        );
    }
...
command_user="traderclaw:traderclaw"
pidfile="/run/${{RC_SVCNAME}}.pid"
umask 027
output_log="/var/log/traderclaw/access.log"
error_log="/var/log/traderclaw/error.log"
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/traderclaw");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo traderclaw service install"
        );
    }

    ensure_traderclaw_user()?;

    let exe = resolve_openrc_executable()?;
    warn_if_binary_in_home(&exe);

    let config_dir = Path::new("/etc/traderclaw");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/traderclaw");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_traderclaw(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by traderclaw:traderclaw)",
    ...
    chown_recursive_to_traderclaw(config_dir)?;
    ...
    let init_path = Path::new("/etc/init.d/traderclaw");
    ...
    run_checked(Command::new("rc-update").args(["add", "traderclaw", "default"]))?;
    println!("✅ Installed OpenRC service: /etc/init.d/traderclaw");
    println!("   Config path: /etc/traderclaw/config.toml");
    println!("   Start with: sudo traderclaw service start");
    let _ = config;
    Ok(())
}
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("traderclaw-daemon.cmd");
...
mod tests {
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/traderclaw.service"));
    }

    #[test]
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "TraderClaw Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        let exe_path = PathBuf::from("/usr/local/bin/traderclaw");
        let script = generate_openrc_script(&exe_path, Path::new("/etc/traderclaw"));

        assert!(script.starts_with("#!/sbin/openrc-run"));
        assert!(script.contains("name=\"traderclaw\""));
        assert!(script.contains("description=\"TraderClaw daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/traderclaw\""));
        assert!(script.contains("command_args=\"--config-dir /etc/traderclaw daemon\""));
        assert!(!script.contains("env TRADERCLAW_CONFIG_DIR"));
        assert!(!script.contains("env TRADERCLAW_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"traderclaw:traderclaw\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/traderclaw/access.log\""));
        assert!(script.contains("error_log=\"/var/log/traderclaw/error.log\""));
    ...
    fn warn_if_binary_in_home_detects_home_path() {
        ...
        let home_path = PathBuf::from("/home/user/.cargo/bin/traderclaw");
        assert!(home_path.to_string_lossy().contains("/home/"));
        assert!(home_path.to_string_lossy().contains(".cargo/bin"));

        let cargo_path = PathBuf::from("/home/user/.cargo/bin/traderclaw");
        assert!(cargo_path.to_string_lossy().contains(".cargo/bin"));

        let system_path = PathBuf::from("/usr/local/bin/traderclaw");
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/traderclaw"), true);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/traderclaw'".to_string()
            ]
        );
    }
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/traderclaw/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/traderclaw/workspace'".to_string(),
                "traderclaw".to_string()
            ]
        );
    }
