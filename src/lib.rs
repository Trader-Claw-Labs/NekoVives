...
Examples:
  ...
  traderclaw channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
Examples:
  ...
  traderclaw channel bind-telegram 123456789")]
    BindTelegram {
...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `TraderClaw` workspace
    Openclaw {
...
Examples:
  ...
  traderclaw cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  traderclaw cron add-every 60000 'Ping heartbeat'     # every minute
  traderclaw cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  ...
  traderclaw cron once 2h 'Follow up on deployment'
  traderclaw cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Command to run
...
Examples:
  traderclaw cron update <task-id> --expression '0 8 * * *'
  traderclaw cron update <task-id> --tz Europe/London --name 'Morning check'
  traderclaw cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  traderclaw hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  traderclaw peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash TraderClaw firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash TraderClaw firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  traderclaw peripheral flash
  traderclaw peripheral flash --port /dev/cu.usbmodem12345
  traderclaw peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash TraderClaw firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
