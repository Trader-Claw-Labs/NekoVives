...
pub async fn run_wizard(force: bool) -> Result<Config> {
    ...
    println!(
        "  {}",
        style("Welcome to TraderClaw — the fastest, smallest AI assistant.")
            .white()
            .bold()
    );
    println!(
    ...
    print_step(3, 9, "Channels (How You Talk to TraderClaw)");
    ...
    if has_channels && config.api_key.is_some() {
        ...
        if launch {
            ...
            std::env::set_var("TRADERCLAW_AUTOSTART_CHANNELS", "1");
        }
    }
...
pub async fn run_channels_repair_wizard() -> Result<Config> {
    ...
    print_step(1, 1, "Channels (How You Talk to TraderClaw)");
...
/// Use `traderclaw onboard` or `traderclaw onboard --api-key sk-... --provider openrouter --memory sqlite|lucid`.
/// Use `traderclaw onboard --interactive` for the full wizard.
...
fn resolve_quick_setup_dirs_with_home(home: &Path) -> (PathBuf, PathBuf) {
    ...
    let config_dir = home.join(".traderclaw");
...
async fn run_quick_setup_with_home(
    ...
    let (traderclaw_dir, workspace_dir) = resolve_quick_setup_dirs_with_home(home);
    ...
    if credential_override.is_none() {
        if provider_supports_keyless_local_usage(&provider_name) {
            println!("    1. Chat:     traderclaw agent -m \"Hello!\"");
            println!("    2. Gateway:  traderclaw gateway");
            println!("    3. Status:   traderclaw status");
            ...
            if canonical_provider_name(&provider_name) == "copilot" {
                ...
                println!(
                    "    1. Login:             traderclaw auth login --provider {}",
                    provider_name
                );
                println!("    2. Chat:              traderclaw agent -m \"Hello!\"");
                println!("    3. Gateway:           traderclaw gateway");
                println!("    4. Status:            traderclaw status");
            }
        } else {
            let env_var = provider_env_var(&provider_name);
            println!("    1. Set your API key:  export {env_var}=\"sk-...\"");
            println!("    2. Or edit:           ~/.traderclaw/config.toml");
            println!("    3. Chat:              traderclaw agent -m \"Hello!\"");
            println!("    4. Gateway:           traderclaw gateway");
        }
    } else {
        println!("    1. Chat:     traderclaw agent -m \"Hello!\"");
        println!("    2. Gateway:  traderclaw gateway");
...
pub async fn run_models_refresh(
    ...
    if !force {
        ...
        .await?
        {
            ...
            println!();
            println!(
                "Tip: run `traderclaw models refresh --force --provider {}` to fetch latest now.",
                provider_name
            );
            return Ok(());
        }
...
pub async fn run_models_list(config: &Config, provider_override: Option<&str>) -> Result<()> {
    ...
    let Some(cached) = cached else {
        println!();
        println!(
            "  No cached models for '{provider_name}'. Run: traderclaw models refresh --provider {provider_name}"
        );
        println!();
        return Ok(());
    };
...
async fn setup_provider(workspace_dir: &Path) -> Result<(String, String, String, Option<String>)> {
    ...
    if providers.is_empty() {
        ...
        print_bullet("TraderClaw works with ANY API that speaks the OpenAI chat completions format.");
    ...
    let api_key = if provider_name == "ollama" {
        ...
        if crate::providers::gemini::GeminiProvider::has_cli_credentials() {
            ...
            print_bullet("TraderClaw will reuse your existing Gemini CLI authentication.");
            println!();
...
fn setup_tool_mode() -> Result<(ComposioConfig, SecretsConfig)> {
    print_bullet("Choose how TraderClaw connects to external apps.");
    ...
    let composio_config = if choice == 1 {
        ...
        print_bullet("TraderClaw uses Composio as a tool — your core agent stays local.");
        println!();
    ...
    // ── Encrypted secrets ──
    println!();
    print_bullet("TraderClaw can encrypt API keys stored in config.toml.");
...
fn setup_hardware() -> Result<HardwareConfig> {
    print_bullet("TraderClaw can talk to physical hardware (LEDs, sensors, motors).");
    ...
    let choice = Select::new()
        .with_prompt("  How should TraderClaw interact with the physical world?")
        .items(&options)
        .default(recommended)
        .interact()?;
...
fn setup_memory() -> Result<MemoryConfig> {
    print_bullet("Choose how TraderClaw stores and searches memories.");
    print_bullet("You can always change this later in config.toml.");
    println!();
...
fn setup_channels() -> Result<ChannelsConfig> {
    print_bullet("Channels let you talk to TraderClaw from anywhere.");
        ...
        match choice {
            ChannelMenuChoice::Telegram => {
                ...
                println!(
                    ...
                    style("— talk to TraderClaw from Telegram").dim()
                );
            ...
            ChannelMenuChoice::Discord => {
                ...
                println!(
                    ...
                    style("— talk to TraderClaw from Discord").dim()
                );
            ...
            ChannelMenuChoice::Slack => {
                ...
                println!(
                    ...
                    style("— talk to TraderClaw from Slack").dim()
                );
            ...
            ChannelMenuChoice::IMessage => {
                ...
                print_bullet("TraderClaw reads your iMessage database and replies via AppleScript.");
                print_bullet(
            ...
            ChannelMenuChoice::WhatsApp => {
                ...
                if mode_idx == 0 {
                    ...
                    let session_path: String = Input::new()
                        ...
                        .default("~/.traderclaw/state/whatsapp-web/session.db".into())
                        .interact_text()?;
                ...
                let verify_token: String = Input::new()
                    ...
                    .default("traderclaw-whatsapp-verify".into())
                    .interact_text()?;
                ...
                config.whatsapp = Some(WhatsAppConfig {
                    ...
                    app_secret: None, // Can be set via TRADERCLAW_WHATSAPP_APP_SECRET env var
                    session_path: None,
                    pair_phone: None,
                    pair_code: None,
                    allowed_numbers,
            ...
            ChannelMenuChoice::Lark | ChannelMenuChoice::Feishu => {
                ...
                println!(
                    ...
                    style(format!("— talk to TraderClaw from {provider_label}")).dim()
                );
                print_bullet(&format!(
            ...
            ChannelMenuChoice::Nostr => {
                ...
                print_bullet("TraderClaw will listen for encrypted DMs on Nostr relays.");
                ...
                // Basic format validation for Nostr key
                let key_trimmed = private_key.trim();
                let key_valid = if key_trimmed.starts_with("nsec1") {
                    key_trimmed.len() > 5
                } else {
                    key_trimmed.len() == 64 && key_trimmed.chars().all(|c| c.is_ascii_hexdigit())
                };
                if key_valid {
                    println!(
                    ...
                    println!(
                        "  {} Invalid private key — expected 64-char hex or nsec1... bech32",
                        style("❌").red().bold()
                    );
                    continue;
                }
...
fn print_summary(config: &Config) {
    ...
    println!(
        "  {}  {}",
        style("⚡").cyan(),
        style("TraderClaw is ready!").white().bold()
    );
    println!(
        "  {}",
        style("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━").cyan()
    ...
    if config.api_key.is_none() && !provider_supports_keyless_local_usage(provider) {
        if provider == "openai-codex" {
            ...
            println!(
                "       {}",
                style("traderclaw auth login --provider openai-codex --device-code").yellow()
            );
            ...
            println!(
                "       {}",
                style(
                    "or: traderclaw auth paste-token --provider anthropic --auth-kind authorization"
                )
                .yellow()
            );
        } else {
    ...
    if has_channels {
        ...
        println!("       {}", style("traderclaw channel start").yellow());
        println!();
        step += 1;
    }
    ...
    println!(
        "       {}",
        style("traderclaw agent -m \"Hello, TraderClaw!\"").yellow()
    );
    println!();
    step += 1;
    ...
    println!("       {}", style("traderclaw agent").yellow());
    println!();
    step += 1;
    ...
    println!("       {}", style("traderclaw status").yellow());
...
mod tests {
    ...
    async fn quick_setup_model_override_persists_to_config_toml() {
        ...
        let _workspace_env = EnvVarGuard::unset("TRADERCLAW_WORKSPACE");
        let _config_env = EnvVarGuard::unset("TRADERCLAW_CONFIG_DIR");
    ...
    async fn quick_setup_existing_config_requires_force_when_non_interactive() {
        ...
        let _config_env = EnvVarGuard::unset("TRADERCLAW_CONFIG_DIR");
        let tmp = TempDir::new().unwrap();
        let traderclaw_dir = tmp.path().join(".traderclaw");
        let config_path = traderclaw_dir.join("config.toml");

        tokio::fs::create_dir_all(&traderclaw_dir).await.unwrap();
    ...
    async fn quick_setup_respects_zero_claw_workspace_env_layout() {
        let _env_guard = env_lock().lock().await;
        let tmp = TempDir::new().unwrap();
        let workspace_root = tmp.path().join("traderclaw-data");
        let workspace_dir = workspace_root.join("workspace");
        let expected_config_path = workspace_root.join(".traderclaw").join("config.toml");

        let _workspace_env = EnvVarGuard::set(
            "TRADERCLAW_WORKSPACE",
        ...
        .await
        .expect("quick setup should honor TRADERCLAW_WORKSPACE");
    ...
    async fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = tokio::fs::read_to_string(tmp.path().join("IDENTITY.md"))
            ...
            .unwrap();
        assert!(
            identity.contains("**Name:** TraderClaw"),
            "should default agent name to TraderClaw"
        );
    ...
    async fn scaffold_handles_special_characters_in_names() {
        ...
        let soul = tokio::fs::read_to_string(tmp.path().join("SOUL.md"))
            .await
            .unwrap();
        assert!(soul.contains("TraderClaw-v2"));
    }
