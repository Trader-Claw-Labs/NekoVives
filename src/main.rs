...
/// `trader-claw` - Rust crypto trading agent for EVM, Solana, TON and Polymarket.
#[derive(Parser, Debug)]
#[command(name = "trader-claw")]
#[command(author = "theonlyhennygod")]
#[command(version)]
#[command(about = "Rust crypto trading agent for EVM, Solana, TON and Polymarket.", long_about = None)]
struct Cli {
...
Examples:
  traderclaw agent                              # interactive session
  traderclaw agent -m \"Summarize today's logs\"  # single message
  traderclaw agent -p anthropic --model claude-sonnet-4-20250514
  traderclaw agent --peripheral nucleo-f401re:/dev/ttyACM0")]
    Agent {
...
Examples:
  traderclaw gateway                  # use config defaults
  traderclaw gateway -p 8080          # listen on port 8080
  traderclaw gateway --host 0.0.0.0   # bind to all interfaces
  traderclaw gateway -p 0             # random available port")]
    Gateway {
...
Launches the full TraderClaw runtime: gateway server, all configured \
channels (Telegram, Discord, Slack, etc.), heartbeat monitor, and \
the cron scheduler. This is the recommended way to run TraderClaw in \
production or as an always-on assistant.

Use 'traderclaw service install' to register the daemon as an OS \
service (systemd/launchd) for auto-start on boot.

Examples:
  traderclaw daemon                   # use config defaults
  traderclaw daemon -p 9090           # gateway on port 9090
  traderclaw daemon --host 127.0.0.1  # localhost only")]
    Daemon {
        /// Port to listen on (use 0 for random available port); defaults to config gateway.port
        #[arg(short, long)]
        port: Option<u16>,
    ...
    /// - `traderclaw estop --level domain-block --domain "*.chase.com"`
    ...
    Estop {
        ...
        /// Level used when engaging estop from `traderclaw estop`.
        #[arg(long, value_enum)]
...
Examples:
  ...
  traderclaw cron add '0 9 * * 1-5' 'Good morning' --tz America/New_York
  traderclaw cron add '*/30 * * * *' 'Check system health'
  traderclaw cron add-at 2025-01-15T14:00:00Z 'Send reminder'
  traderclaw cron add-every 60000 'Ping heartbeat'
  traderclaw cron once 30m 'Run backup in 30 minutes'
  traderclaw cron pause <task-id>
  traderclaw cron update <task-id> --expression '0 8 * * *' --tz Europe/London")]
    Cron {
        #[command(subcommand)]
...
Add, remove, list, and health-check channels that connect TraderClaw \
...
Examples:
  ...
  traderclaw channel add telegram '{\"bot_token\":\"...\",\"name\":\"my-bot\"}'
  traderclaw channel remove my-bot
  traderclaw channel bind-telegram traderclaw_user")]
    Channel {
        #[command(subcommand)]
...
Examples:
  ...
  traderclaw hardware introspect /dev/ttyACM0
  traderclaw hardware info --chip STM32F401RETx")]
    Hardware {
        #[command(subcommand)]
        hardware_command: trader_claw::HardwareCommands,
    },
...
Examples:
  ...
  traderclaw peripheral add nucleo-f401re /dev/ttyACM0
  traderclaw peripheral add rpi-gpio native
  traderclaw peripheral flash --port /dev/cu.usbmodem12345
  traderclaw peripheral flash-nucleo")]
    Peripheral {
        #[command(subcommand)]
        peripheral_command: trader_claw::PeripheralCommands,
    },
...
Examples:
  traderclaw memory stats
  traderclaw memory list
  traderclaw memory list --category core --limit 10
  traderclaw memory get <key>
  traderclaw memory clear --category conversation --yes")]
    Memory {
        #[command(subcommand)]
    ...
    #[command(long_about = "\
Manage TraderClaw configuration.
...
Examples:
  traderclaw config schema              # print JSON Schema to stdout
  traderclaw config schema > schema.json")]
    Config {
        #[command(subcommand)]
    ...
    #[command(long_about = "\
Generate shell completion scripts for `traderclaw`.

The script is printed to stdout so it can be sourced directly:

Examples:
  source <(traderclaw completions bash)
  traderclaw completions zsh > ~/.zfunc/_traderclaw
  traderclaw completions fish > ~/.config/fish/completions/traderclaw.fish")]
    Completions {
        /// Target shell
        #[arg(value_enum)]
...
async fn main() -> Result<()> {
    ...
    if let Some(config_dir) = &cli.config_dir {
        ...
        std::env::set_var("TRADERCLAW_CONFIG_DIR", config_dir);
    }
    ...
    } = &cli.command
    {
        ...
        if std::env::var("TRADERCLAW_AUTOSTART_CHANNELS").as_deref() == Ok("1") {
    ...
    if config.security.otp.enabled {
        ...
        if let Some(uri) = enrollment_uri {
            println!("Initialized OTP secret for TraderClaw.");
    ...
    match cli.command {
        ...
        Commands::Gateway { port, host, .. } => {
            ...
            if port == 0 {
                info!("🚀 Starting TraderClaw Gateway on {host} (random port)");
            } else {
                info!("🚀 Starting TraderClaw Gateway on {host}:{port}");
            }
        ...
        Commands::Daemon { port, host, .. } => {
            ...
            let host = host.unwrap_or_else(|| config.gateway.host.clone());
            if port == 0 {
                info!("🧠 Starting TraderClaw Daemon on {host} (random port)");
            } else {
                info!("🧠 Starting TraderClaw Daemon on {host}:{port}");
            }
            daemon::run(config, host, port).await
        }

        Commands::Status => {
            println!("🦀 TraderClaw Status");
            println!();
                ...
                "gemini" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            ...
                            println!(
                                "Run `traderclaw auth paste-redirect --provider gemini --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
                ...
                "openai-codex" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            println!("Callback capture failed: {e}");
                            println!(
                                "Run `traderclaw auth paste-redirect --provider openai-codex --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
            ...
            match provider.as_str() {
                "openai-codex" => {
                    let pending = load_pending_oauth_login(config, "openai")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending OpenAI login found. Run `traderclaw auth login --provider openai-codex` first."
                        )
                    })?;
                ...
                "gemini" => {
                    let pending = load_pending_oauth_login(config, "gemini")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending Gemini login found. Run `traderclaw auth login --provider gemini` first."
                        )
                    })?;
            ...
            match provider.as_str() {
                "openai-codex" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No OpenAI Codex auth profile found. Run `traderclaw auth login --provider openai-codex`."
                            )
                        }
                    }
                }
                "gemini" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No Gemini auth profile found. Run `traderclaw auth login --provider gemini`."
                            )
                        }
                    }
                }
...
mod tests {
    ...
    fn completions_cli_parses_supported_shells() {
        for shell in ["bash", "fish", "zsh", "powershell", "elvish"] {
            let cli = Cli::try_parse_from(["traderclaw", "completions", shell])
    ...
    fn onboard_cli_accepts_force_flag() {
        let cli = Cli::try_parse_from(["traderclaw", "onboard", "--force"])
    ...
    fn cli_parses_estop_default_engage() {
        let cli = Cli::try_parse_from(["traderclaw", "estop"]).expect("estop command should parse");
    ...
    fn cli_parses_estop_resume_domain() {
        let cli = Cli::try_parse_from(["traderclaw", "estop", "resume", "--domain", "*.chase.com"])
