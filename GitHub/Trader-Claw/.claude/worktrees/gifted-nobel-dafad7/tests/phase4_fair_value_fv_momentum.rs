...
fn fv_estimate_buy_yes_when_price_too_low() {
    let cfg = default_fv_cfg();
    let buf: VecDeque<CandleSnap> = VecDeque::new();
    // YES price = 0.30, but calibration pulls FV toward 0.43 → edge > threshold
    let est = FairValueEngine::estimate(0.30, 0.70, &buf, &cfg);
...
fn fv_estimate_buy_no_when_yes_overpriced() {
    let cfg = default_fv_cfg();
    let buf: VecDeque<CandleSnap> = VecDeque::new();
    // YES price = 0.75, much higher than calibration FV → BuyNo
    let est = FairValueEngine::estimate(0.75, 0.25, &buf, &cfg);
